...
/* EccubePaymentLite42/Controller/Command/CreateOrderFromRegularOrderCommand.php */
class __TwigTemplate_71f5d35a94d8a0f3bd3c3fbbd2e88ddc extends Template
{
    private $source;
    private $macros = [];
    ...
    protected function doDisplay(array $context, array $blocks = [])
    {
        ...
        $__internal_5a27a8ba21ca79b61932376b2fa922d2->enter($__internal_5a27a8ba21ca79b61932376b2fa922d2_prof = new \Twig\Profiler\Profile($this->getTemplateName(), "template", "EccubePaymentLite42/Controller/Command/CreateOrderFromRegularOrderCommand.php"));
        ...
        $__internal_6f47bbe9983af81f1e7450e9a3e3768f->enter($__internal_6f47bbe9983af81f1e7450e9a3e3768f_prof = new \Twig\Profiler\Profile($this->getTemplateName(), "template", "EccubePaymentLite42/Controller/Command/CreateOrderFromRegularOrderCommand.php"));
...
use Plugin\\EccubePaymentLite42\\Service\\GmoEpsilonRequest\\RequestCard3Service;
...
class CreateOrderFromRegularOrderCommand extends Command
{
    ...
    protected function execute(InputInterface \$input, OutputInterface \$output)
    {
        ...
        logs('gmo_epsilon')->info('=== RegularOrder Batch start. ===');
        /** @var Config \$Config */
        ...
        foreach (\$RegularOrders as \$RegularOrder) {
            ...
            if (!is_null(\$Order->getPayment()) &&
                ...
                if (\$userInfoResults['status'] === 'NG') {
                    ...
                    logs('gmo_epsilon')->error('定期ID: '.\$RegularOrder->getId().' エラーコード: '.\$userInfoResults['err_code'].' エラーメッセージ: '.\$userInfoResults['message']);
                    continue;
                }
                if (\$userInfoResults['status'] === 'OK') {
                    if (\$this->isExpireCreditCardService->handle(\$userInfoResults['cardExpire'])) {
                        \$this->changeRegularStatus(\$RegularOrder, RegularStatus::PAYMENT_ERROR);
                        \$PaymentErrorRegularOrders[] = \$RegularOrder;
                        logs('gmo_epsilon')->error('定期ID: '.\$RegularOrder->getId().'について、クレジットカードの有効期限が切れているため、受注の作成・イプシロン決済サービス決済の登録は行っていません。');
                        continue;
                    }
                }
                \$results = \$this
                    ->requestCreateRegularOrderService
                    ->handle(\$RegularOrder, \$Order, 'eccube_payment_lite42_regular_create_command');

                if (\$results['status'] === 'NG') {
                    \$this->changeRegularStatus(\$RegularOrder, RegularStatus::PAYMENT_ERROR);
                    \$PaymentErrorRegularOrders[] = \$RegularOrder;
                    logs('gmo_epsilon')->error('定期ID: '.\$RegularOrder->getId().' エラーコード: '.\$results['err_code'].' エラーメッセージ: '.\$results['message']);
                    continue;
                }
                ...
                if (!\$card3cgiResult) {
                    \$this->changeRegularStatus(\$RegularOrder, RegularStatus::PAYMENT_ERROR);
                    \$PaymentErrorRegularOrders[] = \$RegularOrder;
                    logs('gmo_epsilon')->error('定期ID: '.\$RegularOrder->getId().'について、card3.cgiのリクエスト時に予期せぬエラーが発生しました。');
                    continue;
                }
                \$getSalesResult = \$this
                ...
                if ((int) \$getSalesResult['state'] !== PaymentStatus::CHARGED && (int) \$getSalesResult['state'] !== PaymentStatus::TEMPORARY_SALES) {
                    \$this->changeRegularStatus(\$RegularOrder, RegularStatus::PAYMENT_ERROR);
                    \$PaymentErrorRegularOrders[] = \$RegularOrder;
                    logs('gmo_epsilon')->error('定期ID: '.\$RegularOrder->getId().'について、イプシロン決済サービスに有効な決済が登録されませんでした。 state = '.\$getSalesResult['state']);
                    continue;
                }
        ...
        if (count(\$RegularOrders) === 0) {
            ...
            logs('gmo_epsilon')->info('=== RegularOrder Batch not found target. ===');
        }
        foreach (\$SuccessRegularOrders as \$regularOrder) {
            \$this->io->text('=== Created an order for regular order id = '.\$regularOrder->getId().'. ===');
            logs('gmo_epsilon')->info('=== Created an order for regular order id = '.\$regularOrder->getId().'. ===');
        }
        \$this->io->text('=== RegularOrder Batch end. ===');
        logs('gmo_epsilon')->info('=== RegularOrder Batch end. ===');
    ...
    private function changeRegularStatus(RegularOrder \$RegularOrder, int \$regularStatusId): void
    {
        ...
        logs('gmo_epsilon')->info('=== RegularOrderId = '.\$RegularOrder->getId().' 定期ステータスを'.\$RegularStatus->getName().'に変更しました ===');
    }
...
";
    ...
    {
        return "EccubePaymentLite42/Controller/Command/CreateOrderFromRegularOrderCommand.php";
    }
...
", "EccubePaymentLite42/Controller/Command/CreateOrderFromRegularOrderCommand.php", "C:\\xampp\\htdocs\\ec-cube-dev\\app\\Plugin\\EccubePaymentLite42\\Controller\\Command\\CreateOrderFromRegularOrderCommand.php");
    }
